"use client";

import { Link, useLocation } from "react-router-dom";
import { navigationData, roleBasedNavigation } from "../data/navigationData";

import { LogOut } from "lucide-react";
import { useAuth } from "../context/AuthContext";

function MobileNav({ isMenuOpen, setIsMenuOpen }) {
  const location = useLocation();

  const {
    data,
    isAuthenticated,
    isLoading,
    logout: logoutMutation,
    refetchUser,
  } = useAuth();

  const getRoleBasedNavItems = () => {
    if (!isAuthenticated || !data?.user?.role) return [];
    return roleBasedNavigation[data.user.role] || [];
  };

  const handleLogout = () => {
    logoutMutation.mutate();
  };

  const isActive = (path) => location.pathname === path;

  return (
    <>
      {/* Mobile Navigation Menu */}
      {isMenuOpen && (
        <div className="md:hidden">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-t border-border transition-all duration-200">
            {!isAuthenticated &&
              navigationData.map((item) => (
                <a
                  key={item.name}
                  href={item.href}
                  className={`block px-4 py-3 rounded-lg text-base font-medium transition-all duration-200 ${
                    isActive(item.href)
                      ? "bg-secondary text-secondary-foreground shadow-sm"
                      : "text-muted-foreground hover:text-foreground hover:bg-muted"
                  }`}
                  onClick={() => setIsMenuOpen(false)}
                >
                  {item.name}
                </a>
              ))}
            {isAuthenticated &&
              getRoleBasedNavItems().map((item) => (
                <Link
                  key={item.name}
                  to={item.href}
                  className={`block px-4 py-3 rounded-lg text-base font-medium transition-all duration-200 ${
                    isActive(item.href)
                      ? "bg-secondary text-secondary-foreground shadow-sm"
                      : "text-muted-foreground hover:text-foreground hover:bg-muted"
                  }`}
                  onClick={() => setIsMenuOpen(false)}
                >
                  {item.name}
                </Link>
              ))}
            <div className="pt-4 pb-3 border-t border-border">
              {isLoading ? (
                <div className="px-4 py-3">
                  <div className="w-32 h-8 bg-muted rounded-lg animate-pulse"></div>
                </div>
              ) : isAuthenticated ? (
                <>
                  <Link to={`/profile/${data?.user.id}`}>
                    <div className="px-4 py-3 flex items-center space-x-3 text-foreground hover:text-secondary transition-colors duration-200 rounded-lg hover:bg-muted">
                      {data?.user.imageUrl ? (
                        <img
                          src={data.user.imageUrl || "/placeholder.svg"}
                          alt={data.user.name}
                          className="w-8 h-8 rounded-full object-cover ring-2 ring-border"
                        />
                      ) : (
                        <img
                          src="data:image/png;base64,"
                          alt="Default Avatar"
                          className="w-8 h-8 rounded-full object-cover ring-2 ring-border"
                        />
                      )}
                      <span className="text-sm font-medium">
                        {data?.user.name}
                      </span>
                    </div>
                  </Link>
                  <button
                    onClick={() => {
                      handleLogout();
                      setIsMenuOpen(false);
                    }}
                    disabled={logoutMutation.isPending}
                    className="w-full text-left flex items-center space-x-2 px-4 py-3 rounded-lg text-base font-medium text-muted-foreground hover:text-destructive hover:bg-muted transition-all duration-200 disabled:opacity-50"
                  >
                    <LogOut className="w-4 h-4" />
                    <span>
                      {logoutMutation.isPending ? "Signing out..." : "Sign out"}
                    </span>
                  </button>
                </>
              ) : (
                <>
                  <Link
                    to="/login"
                    className="block px-4 py-3 rounded-lg text-base font-medium text-muted-foreground hover:text-foreground hover:bg-muted transition-all duration-200"
                    onClick={() => setIsMenuOpen(false)}
                  >
                    Sign In
                  </Link>
                  <Link
                    to="/signup"
                    className="block px-4 py-3 rounded-lg text-base font-medium bg-secondary hover:bg-secondary/90 text-secondary-foreground mt-2 transition-all duration-200 shadow-sm"
                    onClick={() => setIsMenuOpen(false)}
                  >
                    Get Started
                  </Link>
                </>
              )}
            </div>
          </div>
        </div>
      )}
    </>
  );
}

export default MobileNav;
