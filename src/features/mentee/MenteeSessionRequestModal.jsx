import { X, User, Calendar, Clock, Users, MessageSquare, ExternalLink, Edit, Trash2 } from "lucide-react"
...
export default function MenteeSessionRequestModal({ request, onClose, onWithdraw, onUpdateAgenda, isLoading }) {
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
            <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-3">Service Information</h3>
            <div className="space-y-2">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="font-medium text-gray-900 dark:text-white">{request.mentorName}</span>
                <span className="text-sm text-gray-500">@{request.mentorId.slice(-8)}</span>
              </div>
              <a
                href={`/profile/${request.mentorId}`}
                target="_blank"
        ...
        {/* Actions */}
        {(onWithdraw || onUpdateAgenda) && (
          <div className="flex items-center justify-end space-x-3 p-6 border-t border-gray-200 dark:border-gray-700">
            {onUpdateAgenda && (
              <button
                onClick={() => {
                  onClose()
                  onUpdateAgenda(request)
                }}
                disabled={isLoading}
                className="inline-flex items-center space-x-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors disabled:opacity-50"
              >
