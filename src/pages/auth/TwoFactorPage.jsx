...
export default function TwoFactorPage() {
  ...
  const { verify2FA: verify2FAMutation, resend2FA: resend2FAMutation } =
    useAuth();
        ...
        <div className="bg-white dark:bg-gray-800 py-8 px-4 shadow sm:rounded-lg sm:px-10">
          ...
          <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
            <div>
              <label
                htmlFor="code"
                className="block text-sm font-medium text-gray-700 dark:text-gray-200 text-center mb-2"
              >
                Enter 6-digit code
              </label>
              <input
                ...
                maxLength="6"
                placeholder="000000"
                className={`w-full px-3 py-3 border rounded-md shadow-sm text-center text-2xl font-mono tracking-widest focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 ${
                  errors.code
                    ? "border-red-300"
            ...
            >
              {isSubmitting || verify2FAMutation.isPending
                ? "Verifying..."
                : "Verify Code"}
            </button>
              ...
              >
                <RefreshCw
                  className={`h-4 w-4 mr-1 ${
                    resend2FAMutation.isPending ? "animate-spin" : ""
                  }`}
                />
          ...
          {resend2FAMutation.error && (
            <div className="mt-4 rounded-md bg-red-50 dark:bg-red-200 p-4">
              <div className="text-sm text-red-700 text-center">
                {resend2FAMutation.error.message}
              </div>
            </div>
          )}
          ...
          <div className="mt-6 text-center">
            <Link
              to="/login"
              className="inline-flex items-center text-sm text-gray-600 dark:text-gray-300 hover:text-gray-500 dark:hover:text-white"
            >
